...
export const FilterSection = ({ onSearch, status }: Props) => {
  const { searchParams } = usePersistedSearch<ISearchParams>('company-country');
  ...
  const onSubmit = async (values: any) => {
    const params: ISearchParams = {
      status:
        values.status?.toString().length > 0 ? values.status.toString() : null,
      countryName:
        values.countryName?.toString().length > 0 ? values.countryName : null,
      countryCode:
        values.countryCode?.toString().length > 0 ? values.countryCode : null,
    };
    onSearch?.(params);
  };
