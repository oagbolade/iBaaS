...
export const BuyFromVault = ({
  ...
  const onSubmit = (
    ...
    if (isSubmitting) {
      ...
      const buyData: VaultManagementValues = {
        ...restValues,
        action: 'buy', // Note This buying from value
        glaccno: glnumber,
  ...
  useEffect(() => {
    const submit = document.getElementById('buySubmitButton');
    if (isSubmitting) {
      submit?.click();
    }
    return () => {
  ...
  return (
    ...
    >
      <Form>
        ...
        <button id="buySubmitButton" type="submit" style={{ display: 'none' }}>
          submit alias
        </button>
      </Form>
    </Formik>
