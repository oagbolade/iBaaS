...
import { useCreateCashDeposit } from '@/api/operation/useCashDeposit';

const mockMutate = jest.fn();

jest.mock('../../../api/operation/useCashDeposit', () => ({
  useCreateCashDeposit: jest.fn(() => ({
    mutate: jest.fn()
  }))
}));
...
describe('CashDeposit Form', () => {
  beforeEach(() => {
    (useCreateCashDeposit as jest.Mock).mockReturnValue({ mutate: mockMutate });
  });
      ...
      <QueryClientProvider client={queryClient}>
        <ToastMessageContext.Provider value={mockToastActions}>
          <Formik initialValues={{}} onSubmit={mockMutate}>
            <CashDeposit currencies={currencies} />
          </Formik>
  ...
  it('Should call mutate function with correct data on form submission', async () => {
    ...
    const debitAccountNumber = screen.getByRole('button', { name: 'Search' });
    ...
    const depositorName = screen.getByLabelText(/depositor's name/i);

    fireEvent.change(depositorName, { target: { value: '100' } });
    ...
    expect(depositorName).toHaveValue('100');
