...
export async function getChequebookStatus(
  ...
  try {
    const urlEndpoint = `/api/ReportServices/Chequebookstatus?status=${params?.status}&startDate=${params?.startDate}&endDate=${params?.endDate}&acctno=${params?.accountNumber || ''}&pageNumber=${Number(params?.pageNumber) || 1}&pageSize=${params?.pageSize || 10}&getAll=${params?.getAll || false}`;
