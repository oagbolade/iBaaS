...
import {
  CreateBulkInsPostBulkFormValues,
  CreateBulkUploadFormValues
...
async function createInsPostBulk(
  toastActions: IToastActions,
  body: CreateBulkInsPostBulkFormValues
): Promise<void> {
  try {
    const urlEndpoint = '/Operations/Operations/InsPostBulk';
    const { data }: AxiosResponse<APIResponse> = await axiosInstance({
      url: urlEndpoint,
      method: 'POST',
      data: { ...body },
...
export function useCreateInsPostBulk() {
  const router = useRouter();
  const toastActions = useContext(ToastMessageContext);

  const { mutate, isPending, isError, error } = useMutation({
    mutationFn: (body: CreateBulkInsPostBulkFormValues) =>
      createInsPostBulk(toastActions, body),
    onSuccess: () => {
      handleRedirect(router, '/operation/cash/');
    }
  });
